...
int main(int argc, char *argv[]){
    ...
    for (int i=0;i<conTramasAux;i++)
    {
        ...
        //Alocamos memoria a fuente y destino
        memset(&A, 0, sizeof(A));
        memset(&B, 0, sizeof(B));
        //diress fuente
        ...
        /*si ya hay direcciones guardadas, se verifican*/
        if(total > 0 )
        {
             /*Se verifica si una de las direcciones guardadas es igual a la fuente de la sig trama*/
            ...
            /*la dirección fuente es repetida / ya guardada */
            if(check != 0)
            {check = 0;                
            }else{
                /* es una nueva dirección fuente, se guarda*/              
                dir[total].dir.sin_addr.s_addr = A.sin_addr.s_addr;                
                dir[total].env = 1;
                dir[total].rec = 0;
                total=total+1;
            }
            /*Se verifica si una de las direcciones guardada es igual a la destino de la sig trama*/
            ...
            /*la dirección destino es repetida / ya guardada */
            if(check != 0)
            {check = 0;                
            }else{
                /* es una nueva dirección destino, se guarda*/              
            ...
            /*No se ha guardado ninguna dirección*/
            //Dirección fuente
            total++;
...
/* Métodos de captura y análisis de frames */
void *Capturador(void *arg){
    ...
    /*Recorrido por número de tramas solicitadas*/
    while(conTramas--)
    {
        /* Se recibe frame */
        ...
        if(n < 0)
        {
            ...
            /*Si el frame es de tamaño válido para ser verificada*/
            if(n > sizeof(struct ethhdr))
            {
                //check2 = valProt(buff2);
                /* Verificar protocolo */
                struct ethhdr *tram2;
                struct iphdr *ip;
                ...
                /* Si no es el solicitado */
                if(check2 != 1){
                    conTramas=conTramas+1;
                }else{            
                    /* Se almacena la información en la estructura */
        ...
        {            
            ...
            if(ntohs(tram2->h_proto) == ETH_P_IP)
            {
                if(byte >= (sizeof(struct ethhdr)+sizeof(struct iphdr)))
                {
                    ...
                    /* Verificación para el tipo de serivicio de ip->tos */
                    int iptos = ip->tos;
